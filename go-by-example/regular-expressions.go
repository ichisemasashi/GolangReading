...
func main() {
	...
	// 他の正規表現タスクでも使うためには、Compile して
	// 最適化された Regexp 構造体を作る必要があります。
	...
	// All がつくバリエーションは、最初のマッチだけでなく、 入力の全てのマッチ部分を対象とします。 例えば、正規表現にマッチする全ての文字列を検索する例は、 次の通りです。
	...
	// これまでの例では、文字列型の引数をもち、MatchString と
	// いった名前の関数を使いました。名前から String を除いた関数に、
	// []byte 型の引数を渡すこともできます。
	fmt.Println(r.Match([]byte("peach")))

	// 正規表現のグローバル変数を作る場合は、Compile の
	// バリエーションとして MustCompile を使えます。 MustCompile
	// はエラーを返す代わりに panic するため、 グローバル変数での
	// 利用をより安全にします。
