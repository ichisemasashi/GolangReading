...
/*
   この例では、状態は 1 つのゴルーチンに所有させます。 これは、データが
   並行アクセスで決して壊れないことを保証します。 ほかのゴルーチンが状態を
   読み書きするためには、 データを所有するゴルーチンにメッセージを送り、
   対応するデータを受け取る必要があります。 readOp 構造体と writeOp 構造体
   は、それらの要求や データをもつゴルーチンが応答する方法をカプセル化します。
*/
type readOp struct {
	key  int
	resp chan int
...
func main() {
	...
	// このゴルーチンは、reads チャネルと writes チャネルを 繰り返し
	...
	// 値を読み込むゴルーチンを 100 個開始します。 各読み込みは、
