...
func main() {
	...
	// その場合は、os.File 値を取得するためにファイルを Open
	// することから始めます。
	f, err := os.Open("/tmp/dat")
	check(err)

	// ファイルの先頭から何バイトか読み込みます。5バイトまでの
	// 読み込みを許しますが、 実際に何バイト読まれたかも記録します。
	b1 := make([]byte, 5)
	n1, err := f.Read(b1)
	check(err)
	...
	// 例えば、前述の例のような読み込みは、ReadAtLeast を使ってより
	// 頑強に実装できます。
	o3, err := f.Seek(6, 0)
	check(err)
	b3 := make([]byte, 2)
