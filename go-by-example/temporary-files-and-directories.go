...
func main() {
	...
	// を実行することです。これは、ファイルを作成し、かつ 読み書き
	// のためにオープンします。この例では、ioutil.TempFile が
	// OS デフォルトの場所にファイルを作成するよう 第1引数に "" を
	// 指定しています。
	f, err := ioutil.TempFile("", "sample")
	check(err)

	// 一時ファイルの名前を表示します。Unix ベースの OS では、
	// ディレクトリは /tmp のようになるでしょう。 ファイル名は、
	// ioutil.TempFile の第2引数で指定された接頭辞から始まり、
	// 残りの部分は並列呼び出しされても常に 異なる名前になるよう
	// 自動的に決定されます。
