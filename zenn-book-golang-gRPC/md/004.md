...
-   `google.golang.org/grpc/cmd/protoc-gen-go-grpc` : `protoc`コマンドがGoのコードを生成するのに利用
...
-   `--go_opt`: `hello.pb.go`ファイル生成時のオプション。 今回は`paths=source_relative`を指定して`--go_out`オプションでの指定が相対パスであることを明示
-   `--go-grpc_out`: `hello_grpc.pb.go`ファイルの出力先ディレクトリを指定
-   `--go-grpc_opt`: `hello_grpc.pb.go`ファイル生成時のオプション。 今回は`paths=source_relative`を指定して`--go-grpc_out`オプションでの指定が相対パスであることを明示
